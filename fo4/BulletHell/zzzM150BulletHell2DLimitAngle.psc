Scriptname zzzM150BulletHell2DLimitAngle extends ObjectReference
...
function ShotAll ()
    ...
    while (shotCountIndex < shotCount)
        ...
        while (fireCount < Angles.length)
            Float r = Angles[fireCount] + playerAngle
            if (360 <= r)
                r -= 360
            endIf
            ArrayX[fireCount] = Math.sin(r) * radius
            ArrayY[fireCount] = Math.cos(r) * radius
            fireCount += 1
        endWhile

        shotSpell(ArrayX, ArrayY, PlayerREF)
...
; 発射---------------------------------------------------------------------
function shotSpell (Float[] ArrayX, Float[] ArrayY, Actor akBlameActor)
    ...
    while (i < ArrayX.length)
        ...
        targets[i].MoveTo(self, ArrayX[i], ArrayY[i], 0)
        i += 1
    endwhile
